...
async function main() {
    ...
    const admin = await prisma.user.upsert({
        ...
        create: {
            ...
            jobType: client_1.JobType.DIRECTOR,
            career: 15,
        },
    });
    ...
    const users = [];
    for (let i = 0; i < 10; i++) {
        const jobTypes = Object.values(client_1.JobType);
    ...
    const categories = Object.values(client_1.PostCategory);
    const posts = [];
    for (let i = 0; i < 100; i++) {
        const category = categories[i % categories.length];
        const titles = TITLES[category];
        const contents = CONTENTS[category];
        const isAnonymous = category === client_1.PostCategory.ANONYMOUS;
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const targetPost = posts[(i + 5) % posts.length];
        if (targetPost.authorId === reporter.id)
            continue;
        try {
            await prisma.report.create({
                data: {
                    type: client_1.ReportType.POST,
                    reason: reportReasons[i % reportReasons.length],
                    reporterId: reporter.id,
                    targetPostId: targetPost.id,
                    status: i < 2 ? client_1.ReportStatus.PENDING : client_1.ReportStatus.REVIEWED,
    ...
    for (let i = 0; i < 3; i++) {
        const reporter = users[i];
        const targetUser = users[(i + 5) % users.length];
        if (targetUser.id === reporter.id)
            continue;
        try {
            await prisma.report.create({
                data: {
                    type: client_1.ReportType.USER,
...
//# sourceMappingURL=seed-mock.js.map