...
export class AdminService {
   ...
   * 자동생성 콘텐츠 목록 조회
   */
  async getAutoContent(page = 1, limit = 20, status?: string) {
    const skip = (page - 1) * limit;
    const where: any = {
      isAutoGenerated: true,
  ...
  /**
   * 자동생성 콘텐츠 승인 (DRAFT → PUBLISHED)
   */
  async approveAutoContent(id: string) {
    const post = await this.prisma.post.findUnique({
      where: { id },
    });
    ...
    if (!post.isAutoGenerated) {
      throw new BadRequestException('자동생성 콘텐츠가 아닙니다');
    }
   ...
   * 자동생성 콘텐츠 거부 (삭제)
   */
  async rejectAutoContent(id: string) {
    const post = await this.prisma.post.findUnique({
      where: { id },
    });
